...
function modify(commentid) {
  ...
  $(".comment" + commentid).html(
    '<div id="target"><textarea id="summernote' +
      commentid +
      '" name="content">' +
      $(".comment" + commentid).html() +
      '</textarea><div class="dropzone" id="modifydropzone"></div><div class="col-md-12"><button type="submit" class="btn btn-success" style="float:right;">기록 수정</button><button class="btn btn-success" style="float:right; margin-right:10px;" onclick="return cancelmodify()">수정 취소</button></div></div>'
  );
  ...
  new Dropzone("div#modifydropzone", {
    ...
    success: function (file, response) {
      var link =
        '<a href="http://dcomstudy.com/download/' +
        response +
        '">' +
        file.name +
        "</a></br>";
  ...
  function sendFile(file, el) {
    ...
    $.ajax({
      ...
      success: function (url) {
        $(el).summernote(
          "editor.insertImage",
          "http://dcomstudy.com/download/" + url
        );
      },
    });
  }
...
function reply(commentid) {
  ...
  // summernote+commentid는 임시 저장소 이다.
  $(".reply" + commentid).html(
    '<div class="panel panel-default" style="margin-left:15px; margin-top:10px;"><div class="panel-heading">답글 작성</div><div class="panel-body"><textarea id="summernote_' +
      commentid +
      '" name="content"></textarea><div class="dropzone" id="replydropzone"></div><div class="col-md-12"><button type="submit" class="btn btn-success" style="float:right;">답글 작성</button></div></div></div>'
  );
...
function modify2(commentid, anonymous) {
  ...
  $(".comment" + commentid).html(
    '<div id="target"><textarea id="summernote' +
      ...
      '" name="content">' +
      $(".comment" + commentid).html() +
      '</textarea><div class="dropzone" id="modifydropzone"></div><div class="col-md-12"><span id="toggle"><input type="checkbox" name="toggle" id="modifyanonymous' +
      commentid +
      '" data-onstyle="success" data-offstyle="danger" ' +
      ischeck +
      ' ><input type="hidden" name="anonymous" value="0"></span><button type="submit" class="btn btn-success" style="float:right;">기록 수정</button><button class="btn btn-success" style="float:right; margin-right:10px;" onclick="return cancelmodify()">수정 취소</button></div></div>'
  );
...
function reply2(commentid) {
  ...
  $(".reply" + commentid).html(
    '<div class="panel panel-default" style="margin-left:15px; margin-top:10px;"><div class="panel-heading">답글 작성</div><div class="panel-body"><textarea id="summernote_' +
      commentid +
      '" name="content"></textarea><div class="dropzone" id="replydropzone"></div><div class="col-md-12"><span id="toggle"><input type="checkbox" checked name="toggle" id="replyanonymous" data-onstyle="success" data-offstyle="danger"><input type="hidden" name="anonymous" value="0"></span><button type="submit" class="btn btn-success" style="float:right;">답글 작성</button></div></div></div>'
  );
...
function search(boardid) {
  var mode = $("#searchfield").val();
  var text = $("#searchtext").val();
  if (!text) location.href = "/" + boardid;
  else location.href = "/" + boardid + "/search/" + mode + "/" + text;
}
